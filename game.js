...
console.log('MU PDL Survival build: v17 - Firebase Auth/Firestore/Realtime Multiplayer');
...
$('logoutBtn').onclick=async()=>{ savePlayerProgressNow(); try{ await signOut(auth); }catch(e){} state.user=null; state.uid=null; state.progressLoaded=false; show('loginScreen'); };
document.querySelectorAll('.world-card').forEach(card=>card.onclick=()=>enterWorld(card.dataset.world));
...
document.addEventListener('visibilitychange',()=>{ if(document.hidden){ rightMouseDown=false; clearInputState(); } });
...
function enterWorld(key){
  currentWorld=WORLD_CONFIG[key]; state.world=key; show('loadingScreen'); $('loadingText').textContent=`Entrando no mundo ${currentWorld.name}...`;
  setTimeout(()=>{ show('gameScreen'); startGame(); },900);
}
...
function startGame(){
  ...
  addWorldEnvironment(); createPlayer(); setupSpawnSlots(); startMultiplayerSync(); animate();
}
function makeGrassTexture(){
  ...
  for(let i=0;i<5000;i++){
    const g=55+Math.floor(Math.random()*90);
    ctx.fillStyle=`rgb(${20+Math.random()*20},${g},${18+Math.random()*20})`;
...
function addSkyAndAtmosphere(){
  ...
  for(let i=0;i<70;i++){
    const star=new THREE.Mesh(new THREE.SphereGeometry(.18+Math.random()*.25,6,4), starMat);
    star.position.set((Math.random()-.5)*360, 75+Math.random()*90, (Math.random()-.5)*360);
    scene.add(star);
  }
}
...
function addRock(x,z,scale=1){
  ...
  rock.position.set(x,.8*scale,z); rock.scale.y=.55+Math.random()*.6; rock.rotation.set(Math.random(),Math.random(),Math.random()); rock.castShadow=true; rock.receiveShadow=true; scene.add(rock); addCollider(x,z,1.0*scale,'rock',rock);
}
...
function addForestAndRocks(){
  for(let i=0;i<180;i++){
    const x=(Math.random()-.5)*450, z=(Math.random()-.5)*450;
    if(Math.hypot(x,z)<52) continue;
    addTree(x,z,.75+Math.random()*1.25);
  }
  for(let i=0;i<85;i++) addRock((Math.random()-.5)*430,(Math.random()-.5)*430,.55+Math.random()*1.6);
  for(let i=0;i<95;i++) addBush((Math.random()-.5)*420,(Math.random()-.5)*420,.45+Math.random()*1.1);
}
function addMountainBorder(){
  const mat=new THREE.MeshStandardMaterial({color:0x403a37, roughness:.98});
  for(let i=0;i<48;i++){
    const angle=(i/48)*Math.PI*2;
    const radius=245+Math.random()*18;
    const h=18+Math.random()*26;
    const m=new THREE.Mesh(new THREE.ConeGeometry(10+Math.random()*10,h,7),mat);
...
function setupSpawnSlots(){
  ...
  spawnPoints.forEach(([x,y,z],idx)=>{
    ...
    for(let i=0;i<count;i++){
      const slot={x:x+(Math.random()*8-4), z:z+(Math.random()*8-4), npc:null, respawnAt:0};
...
function processRespawns(t){
  spawnSlots.forEach(slot=>{
    if(!slot.npc && t>=slot.respawnAt){
      slot.npc=createNpc(slot.x+(Math.random()*6-3), slot.z+(Math.random()*6-3), slot);
    }
  });
}
function createPlayer(){
  ...
  // Arma no lado direito visual do personagem quando a câmera está atrás dele.
  // Como o modelo olha para +Z e a câmera fica atrás, esse lado corresponde ao braço em X negativo.
  weaponMesh.position.set(-.24,-.78,.28);
  leftArm.add(weaponMesh);
  player.userData.weaponArm=leftArm;
...
function createNpc(x,z,slot=null){
  const spec=getNpcSpeciesForSlot(slot);
  ...
  const npcLevel = Math.max(1, state.level + worldBonus + Math.floor(Math.random()*3));
  ...
  Object.assign(npc.userData,{level:npcLevel, damage:getNpcDamageByLevel(npcLevel), attackDelay:spec.attackDelay, lastHit:0, radius:spec.radius, hp:maxHp,maxHp,hpBar,hpGroup,label,slot,walkTime:Math.random()*10,attackAnim:0,attackUntil:0,baseY:0,attackDir:null});
...
function makeRemotePlayerModel(data={}){
  const group=new THREE.Group();
  group.userData={target:new THREE.Vector3(data.x||0,0,data.z||0), targetRot:data.rot||0, walkClock:Math.random()*10, lastX:data.x||0, lastZ:data.z||0, label:null};
  ...
  const sword=new THREE.Mesh(new THREE.BoxGeometry(.08,.95,.08),metal); sword.position.set(.18,-.72,.28); armR.add(sword);
  ...
  group.userData.parts={armL,armR,legL,legR,label};
...
function startMultiplayerSync(){
  ...
  syncOwnPlayerOnline();
  multiplayerSyncTimer=setInterval(syncOwnPlayerOnline, 110);
}

function syncOwnPlayerOnline(){
  if(!multiplayerMyRef || !player || !state.uid || playerDead) return;
...
function movePlayer(delta=0.016){
  ...
  sprintActive = !!(keys.ShiftLeft && keys.KeyW && moveForward > 0 && state.energy > 1);
  ...
  const moving = moveForward !== 0;
  if(moveForward !== 0){
    const speed = PLAYER_MOVE_SPEED * (sprintActive && moveForward > 0 ? SPRINT_MULTIPLIER : 1);
    tryMoveObject(player, forward.x * moveForward * speed, forward.z * moveForward * speed, PLAYER_RADIUS, player);
...
function moveNpcs(){
  ...
  npcs.forEach(n=>{
    const v=new THREE.Vector3().subVectors(player.position,n.position); const d=v.length();
    let moving=false;
    if(d<18 && d>1.45){
      v.normalize();
      ...
      // Os modelos dos NPCs olham para -Z; somamos PI para eles não andarem de costas.
    ...
    animateNpcAttack(n);
    if(d<1.35){
      const face=new THREE.Vector3().subVectors(player.position,n.position);
      if(face.lengthSq()>0.0001) n.rotation.y=Math.atan2(face.x,face.z)+Math.PI;
    }
    if(d<1.35 && now-(n.userData.lastHit||0) > n.userData.attackDelay){
...
function stopGame(){ stopMultiplayerSync(true); savePlayerProgressNow(); if(animId) cancelAnimationFrame(animId); actionTimers.forEach(id=>clearInterval(id)); actionTimers=[]; if(renderer){ renderer.dispose(); } npcs=[]; coinPickups=[]; visualEffects=[]; colliders=[]; spawnSlots=[]; scene=null; camera=null; renderer=null; player=null; isAttacking=false; skillLock=false; hudHidden=false; walkClock=0; lastFrameTime=0; cameraYaw=0; cameraLockedDuringSkill=false; rightMouseDown=false; lockedTarget=null; targetIndicator=null; sprintActive=false; sprintEndAt=0; sprintCooldownUntil=0; playerDead=false; deathStartedAt=0; cameraShakeUntil=0; cameraShakePower=0; keys={}; document.body.classList.remove('hud-hidden'); $('combatLog').innerHTML=''; }
