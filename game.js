...
let mobileInput={active:false,x:0,y:0,forward:0,turn:0,mag:0}; mobileCameraTurn=0;
let mobileCameraOffset=0, mobileCameraTurn=0;
...
function setupMobileControls(){
  ...
  const lobbyBtn=$('mobileLobbyBtn');
  const attackBtn=$('mobileAttackBtn');
  const moveLeftBtn=$('mobileMoveLeftBtn');
  const moveRightBtn=$('mobileMoveRightBtn');
  const camLeftBtn=$('mobileCameraLeftBtn');
  const camRightBtn=$('mobileCameraRightBtn');
  ...
  if(lobbyBtn){
    const goLobby=e=>{ e.preventDefault(); stopGame(); show('lobbyScreen'); };
    lobbyBtn.addEventListener('click',goLobby);
    lobbyBtn.addEventListener('touchend',goLobby,{passive:false});
  }
  ...
  bindHold(moveLeftBtn, ()=>{ keys.KeyA=true; }, ()=>{ keys.KeyA=false; });
  bindHold(moveRightBtn, ()=>{ keys.KeyD=true; }, ()=>{ keys.KeyD=false; });
  bindHold(camLeftBtn, ()=>{ mobileCameraTurn=1; }, ()=>{ if(mobileCameraTurn===1) mobileCameraTurn=0; });
  bindHold(camRightBtn, ()=>{ mobileCameraTurn=-1; }, ()=>{ if(mobileCameraTurn===-1) mobileCameraTurn=0; });
}
setupMobileControls();
...
function updateThirdPersonCamera(){
  ...
  if(!cameraLockedDuringSkill){
    if(isMobileLike() && mobileCameraTurn){
      mobileCameraOffset += mobileCameraTurn * 0.025;
    }
    const targetYaw = player.rotation.y + mobileCameraOffset;
    const diff = Math.atan2(Math.sin(targetYaw - cameraYaw), Math.cos(targetYaw - cameraYaw));
