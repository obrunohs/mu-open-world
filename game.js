...
import { getDatabase, ref as dbRef, set as dbSet, onValue, onDisconnect, remove as dbRemove, serverTimestamp as rtdbServerTimestamp, runTransaction } from "https://www.gstatic.com/firebasejs/12.14.0/firebase-database.js";
...
console.log('MU PDL Survival build: v22 - mobile dynamic joystick fix + network patch');
...
let multiplayerDropsRef=null, multiplayerDropsUnsub=null, dropsCleanupTimer=null;
...
let multiplayerActionSeq=0, lastActionType='idle', lastActionAt=0, playerDeathSeq=0, playerRespawnSeq=0;
let currentRegionId=null, regionBannerEl=null, decorAnimatables=[];
...
const LORENCIA_SAFE_ZONE = { id:'safe', name:'Praça Central', subtitle:'Área Segura', x:0, z:0, radius:32 };
// Lorencia Sombria redesenhada: cidade murada no centro e quatro regiões cercadas.
// Cada região tem uma única entrada aberta voltada para a Praça Central.
const LORENCIA_REGIONS = [
  { id:'south_forest', name:'Floresta Sul', subtitle:'NPC nível 1-3', minLevel:1, maxLevel:3, species:['goblin','spider'], x:0, z:112, radius:44, halfX:58, halfZ:46, gateSide:'south', color:0x224d25 },
  { id:'central_forest', name:'Floresta Central', subtitle:'NPC nível 3-5', minLevel:3, maxLevel:5, species:['goblin','wolf','spider'], x:-112, z:0, radius:45, halfX:50, halfZ:58, gateSide:'east', color:0x173d21 },
  { id:'ruins', name:'Ruínas Antigas', subtitle:'NPC nível 5-8', minLevel:5, maxLevel:8, species:['goblin','wolf'], x:112, z:0, radius:45, halfX:50, halfZ:58, gateSide:'west', color:0x574f46 },
  { id:'cemetery', name:'Cemitério Maldito', subtitle:'NPC nível 8-12', minLevel:8, maxLevel:12, species:['wolf','spider','goblin'], x:0, z:-116, radius:48, halfX:60, halfZ:50, gateSide:'north', color:0x2b2832 }
];
...
function isInsideSafeZone(pos){
  if(!pos) return false;
  return Math.hypot(pos.x-LORENCIA_SAFE_ZONE.x, pos.z-LORENCIA_SAFE_ZONE.z) <= LORENCIA_SAFE_ZONE.radius;
}
function getLorenciaRegionAt(x,z){
  if(state.world==='lorencia' && Math.hypot(x-LORENCIA_SAFE_ZONE.x,z-LORENCIA_SAFE_ZONE.z)<=LORENCIA_SAFE_ZONE.radius) return LORENCIA_SAFE_ZONE;
  if(state.world==='lorencia'){
    for(const r of LORENCIA_REGIONS){ if(Math.hypot(x-r.x,z-r.z)<=r.radius) return r; }
  }
  return { id:'wilds', name: currentWorld?.name || 'Mundo', subtitle:'Campo Aberto' };
}
function ensureRegionBanner(){
  if(regionBannerEl) return regionBannerEl;
  regionBannerEl=document.createElement('div');
  regionBannerEl.id='regionBanner';
  Object.assign(regionBannerEl.style,{position:'fixed',left:'50%',top:'16%',transform:'translateX(-50%)',zIndex:'50',padding:'12px 30px',border:'1px solid rgba(255,210,110,.75)',borderRadius:'14px',background:'linear-gradient(90deg, rgba(20,10,22,.05), rgba(20,10,22,.82), rgba(20,10,22,.05))',color:'#ffd56b',font:'bold 24px Arial',letterSpacing:'2px',textAlign:'center',pointerEvents:'none',opacity:'0',transition:'opacity .35s ease, transform .35s ease',textShadow:'0 0 12px rgba(255,170,40,.9)'});
  const sub=document.createElement('div'); sub.className='region-sub'; Object.assign(sub.style,{font:'bold 13px Arial',color:'#dff7ff',letterSpacing:'1px',marginTop:'4px'}); regionBannerEl.appendChild(document.createElement('div')); regionBannerEl.appendChild(sub);
  document.body.appendChild(regionBannerEl);
  return regionBannerEl;
}
function showRegionBanner(region){
  const el=ensureRegionBanner();
  el.children[0].textContent=region.name || '';
  el.children[1].textContent=region.subtitle || '';
  el.style.opacity='1'; el.style.transform='translateX(-50%) scale(1.02)';
  clearTimeout(el._hideTimer);
  el._hideTimer=setTimeout(()=>{ el.style.opacity='0'; el.style.transform='translateX(-50%) scale(.98)'; },2200);
}
function updatePlayerRegion(){
  if(!player) return;
  const r=getLorenciaRegionAt(player.position.x, player.position.z);
  if(r.id!==currentRegionId){ currentRegionId=r.id; showRegionBanner(r); }
}
...
function setupMobileControls(){
  ...
  const JOY_MAX = 54;
  function placeDynamicStick(x,y){
  ...
  function resetStick(){
    ...
    mobileInput={active:false,x:0,y:0,forward:0,turn:0,mag:0,desiredYaw:null};
    if(knob) knob.style.transform='translate(-50%,-50%)';
    const joy=$('mobileJoystick'); if(joy) joy.style.opacity='0';
    stickId=null;
  }
  if(stickZone){
    stickZone.addEventListener('touchstart',e=>{
      ...
      placeDynamicStick(center.x, center.y);
      if(knob) knob.style.transform='translate(-50%,-50%)';
      e.preventDefault();
    }, {passive:false});
    stickZone.addEventListener('touchmove',e=>{
      ...
      const k=len>0 ? Math.min(1,len/JOY_MAX) : 0;
      const nx=len>0 ? dx/len*JOY_MAX*k : 0;
      const ny=len>0 ? dy/len*JOY_MAX*k : 0;
      if(knob) knob.style.transform=`translate(calc(-50% + ${nx}px), calc(-50% + ${ny}px))`;
      const dead=.16;
      const ax = k>dead ? THREE.MathUtils.clamp(dx/JOY_MAX, -1, 1) : 0;
      const ay = k>dead ? THREE.MathUtils.clamp(dy/JOY_MAX, -1, 1) : 0;
      const mag = k>dead ? k : 0;
      mobileInput.active = mag > 0;
      mobileInput.x = ax;
      mobileInput.y = ay;
      mobileInput.turn = 0;
      mobileInput.forward = mag;
      mobileInput.mag = mag;
      // Direção relativa à câmera: dedo para cima anda para frente na tela; lados não ficam invertidos.
      mobileInput.desiredYaw = mag>0 ? cameraYaw + Math.atan2(-ax, -ay) : null;
      keys.KeyW = mag > .18;
      keys.KeyS = false;
      keys.KeyA = false;
      keys.KeyD = false;
...
function startGame(){
  ...
  colliders=[]; decorAnimatables=[]; currentRegionId=null;
  ...
  addWorldEnvironment(); createPlayer(); setupSpawnSlots(); startMultiplayerSync(); startNpcRealtimeSync(); startDropRealtimeSync(); animate(); resizeGameRenderer();
}
...
function addTorch(x,z){
  ...
  pole.position.y=1.05; group.add(pole);
  const bowl=new THREE.Mesh(new THREE.CylinderGeometry(.28,.22,.16,12), new THREE.MeshStandardMaterial({color:0x242020, metalness:.4, roughness:.6}));
  bowl.position.y=2.12; group.add(bowl);
  const flame=new THREE.Mesh(new THREE.ConeGeometry(.18,.55,10), new THREE.MeshBasicMaterial({color:0xff9d24, transparent:true, opacity:.92}));
  flame.position.y=2.48; group.add(flame);
  const flame2=new THREE.Mesh(new THREE.ConeGeometry(.11,.38,8), new THREE.MeshBasicMaterial({color:0xffe26b, transparent:true, opacity:.85}));
  flame2.position.y=2.52; group.add(flame2);
  const light=new THREE.PointLight(0xff9d35,.75,13); light.position.y=2.55; group.add(light);
  group.position.set(x,0,z); scene.add(group); addCollider(x,z,.45,'torch',group);
  decorAnimatables.push({type:'torch', flame, flame2, light, seed:worldRng()*10});
}
function addCrow(x,z,rot=0){
  const group=new THREE.Group();
  const black=new THREE.MeshStandardMaterial({color:0x08080b, roughness:.7});
  const beakMat=new THREE.MeshStandardMaterial({color:0x2f2615, roughness:.6});
  const body=new THREE.Mesh(new THREE.SphereGeometry(.28,10,8),black); body.scale.set(1.15,.7,.78); body.position.y=.55; group.add(body);
  const head=new THREE.Mesh(new THREE.SphereGeometry(.16,10,8),black); head.position.set(0,.72,-.26); group.add(head);
  const beak=new THREE.Mesh(new THREE.ConeGeometry(.045,.18,6),beakMat); beak.rotation.x=Math.PI/2; beak.position.set(0,.72,-.42); group.add(beak);
  const wingL=new THREE.Mesh(new THREE.BoxGeometry(.08,.32,.38),black); wingL.position.set(-.28,.52,0); wingL.rotation.z=.38; group.add(wingL);
  const wingR=wingL.clone(); wingR.position.x=.28; wingR.rotation.z=-.38; group.add(wingR);
  group.position.set(x,0,z); group.rotation.y=rot; scene.add(group);
  decorAnimatables.push({type:'crow', group, head, seed:worldRng()*10});
}
function addBrokenColumn(x,z,scale=1){
  const mat=new THREE.MeshStandardMaterial({color:0x8b8376, roughness:.9});
  const col=new THREE.Mesh(new THREE.CylinderGeometry(.42*scale,.48*scale,2.2*scale,12),mat); col.position.set(x,1.1*scale,z); col.rotation.z=(worldRng()-.5)*.22; scene.add(col); addCollider(x,z,.65*scale,'ruin',col);
  const cap=new THREE.Mesh(new THREE.BoxGeometry(1.2*scale,.25*scale,1.2*scale),mat); cap.position.set(x,2.32*scale,z); cap.rotation.y=worldRng()*Math.PI; scene.add(cap);
}
function addTomb(x,z,rot=0){
  const mat=new THREE.MeshStandardMaterial({color:0x5d5a5b, roughness:.95});
  const base=new THREE.Mesh(new THREE.BoxGeometry(.95,.16,1.55),mat); base.position.set(x,.10,z); base.rotation.y=rot; scene.add(base); addCollider(x,z,.8,'tomb',base);
  const stone=new THREE.Mesh(new THREE.BoxGeometry(.55,1.1,.18),mat); stone.position.set(x,.7,z-.48*Math.cos(rot)); stone.rotation.y=rot; scene.add(stone);
  const crossMat=new THREE.MeshStandardMaterial({color:0x777273, roughness:.95});
  const graveCross=new THREE.Group();
  const longBar=new THREE.Mesh(new THREE.BoxGeometry(.16,.08,1.05),crossMat); longBar.position.y=.08; graveCross.add(longBar);
  const shortBar=new THREE.Mesh(new THREE.BoxGeometry(.72,.08,.14),crossMat); shortBar.position.set(0,.095,-.14); graveCross.add(shortBar);
  graveCross.position.set(x,.22,z+.08); graveCross.rotation.y=rot; scene.add(graveCross);
}
function addMushroom(x,z,scale=1){
  const stem=new THREE.Mesh(new THREE.CylinderGeometry(.05*scale,.08*scale,.35*scale,8),new THREE.MeshStandardMaterial({color:0xd9c5a2})); stem.position.set(x,.18*scale,z); scene.add(stem);
  const cap=new THREE.Mesh(new THREE.SphereGeometry(.18*scale,10,6),new THREE.MeshStandardMaterial({color:0x8d2b38, roughness:.8})); cap.position.set(x,.39*scale,z); cap.scale.y=.45; scene.add(cap);
}
function updateDecorAnimations(t=0){
  const time=t*.001;
  decorAnimatables.forEach(d=>{
    if(d.type==='torch'){
      const p=.85+Math.sin(time*8+d.seed)*.18+Math.sin(time*15+d.seed)*.07;
      d.flame.scale.set(p,1.0+Math.sin(time*10+d.seed)*.12,p);
      d.flame2.scale.set(1.1-p*.12,1.0+Math.sin(time*13+d.seed)*.18,1.1-p*.12);
      d.light.intensity=.55+Math.abs(Math.sin(time*7+d.seed))*.55;
    }else if(d.type==='crow'){
      d.head.rotation.y=Math.sin(time*1.8+d.seed)*.65;
      d.group.position.y=Math.sin(time*1.2+d.seed)*.025;
    }
  });
}
function addWorldEnvironment(){
  ...
  if(state.world==='lorencia'){
    addLorenciaRegions();
    addLorenciaWallsAndPortals();
  }
  addForestAndRocks();
  addMountainBorder();
}
function addLorenciaTown(){
  // Praça Central limpa: área segura para respawn, conversa e futuro trade.
  const plazaMat=new THREE.MeshStandardMaterial({color:0x80745e, roughness:.92});
  const plaza=new THREE.Mesh(new THREE.CylinderGeometry(24,24,.16,72), plazaMat);
  plaza.position.set(0,.08,0); plaza.receiveShadow=true; scene.add(plaza);

  const inner=new THREE.Mesh(new THREE.RingGeometry(7.5,8.1,64), new THREE.MeshBasicMaterial({color:0xcaa85a, transparent:true, opacity:.22, side:THREE.DoubleSide}));
  inner.rotation.x=-Math.PI/2; inner.position.y=.12; scene.add(inner);

  const fountain=new THREE.Group();
  const stoneMat=new THREE.MeshStandardMaterial({color:0x74706b, roughness:.75});
  const basin=new THREE.Mesh(new THREE.CylinderGeometry(3.1,3.5,.45,40), stoneMat); basin.position.y=.34; fountain.add(basin);
  const water=new THREE.Mesh(new THREE.CylinderGeometry(2.55,2.55,.08,40), new THREE.MeshStandardMaterial({color:0x3fb5ff, emissive:0x0b4770, emissiveIntensity:.36, transparent:true, opacity:.72})); water.position.y=.62; fountain.add(water);
  const statue=new THREE.Mesh(new THREE.ConeGeometry(.72,2.25,16), new THREE.MeshStandardMaterial({color:0xa9a19a, roughness:.55})); statue.position.y=1.75; fountain.add(statue);
  fountain.position.set(0,0,0); enableShadows(fountain); scene.add(fountain); addCollider(0,0,3.6,'fountain',fountain);

  // Apenas decoração essencial dentro da cidade para não pesar nem bloquear o spawn.
  [[-17,-17],[17,-17],[-17,17],[17,17],[0,25],[0,-25]].forEach(([x,z])=>addTorch(x,z));
  [[-13,9,1.2],[14,10,-1.2],[-18,-7,.5],[18,-9,-.7]].forEach(([x,z,r])=>addCrow(x,z,r));

  const label=makeTextSprite('PRAÇA CENTRAL',{color:'#9fe7ff',width:4.6,height:1,size:24});
  label.position.set(0,6.3,0); scene.add(label);
}

function addRegionGroundCircle(region){
  const mat=new THREE.MeshStandardMaterial({color:region.color||0x2a3a2a, transparent:true, opacity:.18, roughness:1});
  const mesh=new THREE.Mesh(new THREE.CircleGeometry(region.radius,64),mat);
  mesh.rotation.x=-Math.PI/2; mesh.position.set(region.x,.018,region.z); scene.add(mesh);
}
function addLorenciaRegions(){
  LORENCIA_REGIONS.forEach(addRegionGroundCircle);
  const rng=createSeededRandom('lorencia-region-decor-v3');

  // Marca visual discreta da safe zone.
  const safeRing=new THREE.Mesh(new THREE.RingGeometry(LORENCIA_SAFE_ZONE.radius-.35, LORENCIA_SAFE_ZONE.radius+.35,96), new THREE.MeshBasicMaterial({color:0x82d7ff, transparent:true, opacity:.12, side:THREE.DoubleSide}));
  safeRing.rotation.x=-Math.PI/2; safeRing.position.y=.035; scene.add(safeRing);

  // Floresta Sul: mais aberta e leve, para level 1-3.
  for(let i=0;i<12;i++){ const x=-42+rng()*84, z=76+rng()*56; if(isLorenciaNoNpcZone(x,z)) continue; addTree(x,z,.55+rng()*.55); }
  for(let i=0;i<22;i++){ const x=-46+rng()*92, z=74+rng()*62; if(isLorenciaNoNpcZone(x,z)) continue; addMushroom(x,z,.7+rng()*1.1); }
  for(let i=0;i<5;i++){ const log=new THREE.Mesh(new THREE.CylinderGeometry(.24,.3,3.2,8),new THREE.MeshStandardMaterial({color:0x4d2f19,roughness:.9})); log.rotation.z=Math.PI/2; log.rotation.y=rng()*Math.PI; log.position.set(-38+rng()*76,.32,80+rng()*54); scene.add(log); addCollider(log.position.x,log.position.z,1.4,'fallen_log',log); }

  // Floresta Central: mais fechada, mas com quantidade controlada para FPS.
  for(let i=0;i<16;i++){ const x=-145+rng()*66, z=-42+rng()*84; if(isLorenciaNoNpcZone(x,z)) continue; addTree(x,z,.75+rng()*.85); }
  for(let i=0;i<8;i++){ const barrel=new THREE.Mesh(new THREE.CylinderGeometry(.42,.42,.8,12),new THREE.MeshStandardMaterial({color:0x5a351b,roughness:.78})); barrel.position.set(-142+rng()*58,.4,-35+rng()*70); barrel.rotation.z=(rng()-.5)*.35; scene.add(barrel); addCollider(barrel.position.x,barrel.position.z,.55,'barrel',barrel); }
  addTorch(-88,-34); addTorch(-143,24); addCrow(-126,34,-.5);

  // Ruínas: colunas quebradas e fogo azul.
  for(let i=0;i<18;i++){ const x=80+rng()*65, z=-42+rng()*84; if(isLorenciaNoNpcZone(x,z)) continue; addBrokenColumn(x,z,.62+rng()*.8); }
  for(let i=0;i<5;i++){ const torchX=86+rng()*52, torchZ=-35+rng()*70; if(isLorenciaNoNpcZone(torchX,torchZ)) continue; addTorch(torchX,torchZ); const last=decorAnimatables[decorAnimatables.length-1]; if(last?.flame){ last.flame.material.color.setHex(0x3ca7ff); last.flame2.material.color.setHex(0xb9f2ff); last.light.color.setHex(0x4fb9ff); } }

  // Cemitério: cruzes/túmulos no chão, árvores mortas e corvos.
  for(let i=0;i<34;i++){ const x=-48+rng()*96, z=-153+rng()*74; if(isLorenciaNoNpcZone(x,z)) continue; addTomb(x,z,rng()*Math.PI); }
  for(let i=0;i<8;i++){ const x=-52+rng()*104, z=-158+rng()*82; if(isLorenciaNoNpcZone(x,z)) continue; const dead=new THREE.Mesh(new THREE.CylinderGeometry(.16,.23,3.0,7),new THREE.MeshStandardMaterial({color:0x2b201b,roughness:.95})); dead.position.set(x,1.5,z); dead.rotation.z=(rng()-.5)*.28; scene.add(dead); addCollider(x,z,.42,'dead_tree',dead); }
  [[-22,-104,.2],[22,-128,-.9],[8,-153,1.5],[-34,-138,2.3]].forEach(([x,z,r])=>addCrow(x,z,r));
}
...
function distancePointToSegment(px,pz,x1,z1,x2,z2){
  ...
  const t=Math.max(0,Math.min(1,((px-x1)*vx+(pz-z1)*vz)/len2));
  const x=x1+vx*t, z=z1+vz*t;
  return Math.hypot(px-x,pz-z);
}
function addWallSegment(x1,z1,x2,z2,opts={}){
  const height=opts.height||7.5, thickness=opts.thickness||2.2;
  const color=opts.color||0x3a3431;
  ...
  const step=Math.max(2.8, thickness*1.6);
  const parts=Math.ceil(len/step);
  for(let i=0;i<=parts;i++){
    const t=i/parts;
    const x=x1+(x2-x1)*t, z=z1+(z2-z1)*t;
    addCollider(x,z,thickness*1.08,'wall',wall);
  }
  return wall;
}
function addRoadSegment(x1,z1,x2,z2,width=10,color=0x5a5148){
  const len=Math.hypot(x2-x1,z2-z1);
  if(len<.1) return;
  const road=new THREE.Mesh(new THREE.PlaneGeometry(width,len), new THREE.MeshStandardMaterial({color, roughness:.95, transparent:true, opacity:.82}));
  road.rotation.x=-Math.PI/2;
  road.rotation.z=-Math.atan2(x2-x1,z2-z1);
  road.position.set((x1+x2)/2,.026,(z1+z2)/2);
  road.receiveShadow=true; scene.add(road);
}
function addPortalMonument(x,z,dir,type,labelText){
  const group=new THREE.Group();
  const stone=new THREE.MeshStandardMaterial({color:0x6d6258, roughness:.88});
  const dark=new THREE.MeshStandardMaterial({color:0x2f2929, roughness:.82});
  const glow=new THREE.MeshBasicMaterial({color:type==='cross'?0x805cff:0x7ad7ff, transparent:true, opacity:.34});
  const postOffset=7.2;
  [-postOffset,postOffset].forEach(px=>{
    const post=new THREE.Mesh(new THREE.BoxGeometry(1.8,6.5,1.8),stone); post.position.set(px,3.25,0); group.add(post);
    const cap=new THREE.Mesh(new THREE.ConeGeometry(1.35,1.5,4),stone); cap.position.set(px,7.1,0); cap.rotation.y=Math.PI/4; group.add(cap);
  });
  const top=new THREE.Mesh(new THREE.BoxGeometry(postOffset*2+2.0,1.1,1.8),stone); top.position.y=6.2; group.add(top);
  const ring=new THREE.Mesh(new THREE.TorusGeometry(2.05,.12,8,32),glow); ring.position.y=3.35; ring.rotation.y=Math.PI/2; group.add(ring);
  if(type==='wolf'){
    const wolf=new THREE.Group();
    const body=new THREE.Mesh(new THREE.CapsuleGeometry(.35,1.25,6,10),dark); body.rotation.x=Math.PI/2; body.position.y=1.15; wolf.add(body);
    const head=new THREE.Mesh(new THREE.ConeGeometry(.38,.85,9),dark); head.rotation.x=-Math.PI/2; head.position.set(0,1.25,-.95); wolf.add(head);
    const tail=new THREE.Mesh(new THREE.ConeGeometry(.14,.85,8),dark); tail.rotation.x=1.1; tail.position.set(0,1.25,.9); wolf.add(tail);
    wolf.position.set(0,1.05,-1.55); wolf.scale.set(1.35,1.35,1.35); group.add(wolf);
  }else if(type==='goblin'){
    const g=new THREE.Group();
    const body=new THREE.Mesh(new THREE.CapsuleGeometry(.36,1.1,5,10),dark); body.position.y=1.35; g.add(body);
    const head=new THREE.Mesh(new THREE.SphereGeometry(.34,12,8),dark); head.position.y=2.25; g.add(head);
    const sword=new THREE.Mesh(new THREE.BoxGeometry(.14,1.7,.14),stone); sword.position.set(.55,1.65,-.2); sword.rotation.z=-.45; g.add(sword);
    g.position.set(0,.65,-1.55); g.scale.set(1.25,1.25,1.25); group.add(g);
  }else if(type==='guardian'){
    const guard=new THREE.Mesh(new THREE.CylinderGeometry(.75,.95,3.2,8),dark); guard.position.set(0,2.3,-1.55); group.add(guard);
    const head=new THREE.Mesh(new THREE.BoxGeometry(1.1,.8,1.1),dark); head.position.set(0,4.2,-1.55); group.add(head);
  }else if(type==='cross'){
    const v=new THREE.Mesh(new THREE.BoxGeometry(.5,4.0,.5),dark); v.position.set(0,2.5,-1.55); group.add(v);
    const h=new THREE.Mesh(new THREE.BoxGeometry(2.4,.45,.45),dark); h.position.set(0,3.4,-1.55); group.add(h);
  }
  group.position.set(x,0,z); group.rotation.y=dir; enableShadows(group); scene.add(group);
  // Colisão só nos pilares para o vão do portal ficar aberto.
  const ca=Math.cos(dir), sa=Math.sin(dir);
  [-postOffset,postOffset].forEach(px=>addCollider(x+px*ca,z-px*sa,1.25,'portal_pillar',group));
  const label=makeTextSprite(labelText,{color:'#ffd36b',width:4.5,height:1,size:22}); label.position.set(x,7.4,z); scene.add(label);
}

function addLorenciaWallsAndPortals(){
  // Muro externo alto do mapa inteiro.
  const m=228;
  addWallSegment(-m,-m,m,-m,{height:10,thickness:3.2,color:0x2f2a29});
  addWallSegment(-m,m,m,m,{height:10,thickness:3.2,color:0x2f2a29});
  addWallSegment(-m,-m,-m,m,{height:10,thickness:3.2,color:0x2f2a29});
  addWallSegment(m,-m,m,m,{height:10,thickness:3.2,color:0x2f2a29});

  // Praça Central murada. Os muros encostam nos pilares dos portais, sem frestas.
  const h=38, gap=7.2;
  addWallSegment(-h,-h,-gap,-h,{height:8.2,thickness:2.6,color:0x3b3430}); addWallSegment(gap,-h,h,-h,{height:8.2,thickness:2.6,color:0x3b3430});
  addWallSegment(-h,h,-gap,h,{height:8.2,thickness:2.6,color:0x3b3430}); addWallSegment(gap,h,h,h,{height:8.2,thickness:2.6,color:0x3b3430});
  addWallSegment(-h,-h,-h,-gap,{height:8.2,thickness:2.6,color:0x3b3430}); addWallSegment(-h,gap,-h,h,{height:8.2,thickness:2.6,color:0x3b3430});
  addWallSegment(h,-h,h,-gap,{height:8.2,thickness:2.6,color:0x3b3430}); addWallSegment(h,gap,h,h,{height:8.2,thickness:2.6,color:0x3b3430});

  // Estradas principais. Não recebem spawn de NPC.
  addRoadSegment(0,38,0,66,10);       // Praça -> Floresta Sul
  addRoadSegment(-38,0,-64,0,10);     // Praça -> Floresta Central
  addRoadSegment(38,0,64,0,10);       // Praça -> Ruínas
  addRoadSegment(0,-38,0,-66,10);     // Praça -> Cemitério

  // Portais da Praça Central.
  addPortalMonument(0,38,0,'wolf','Floresta Sul');
  addPortalMonument(-38,0,Math.PI/2,'goblin','Floresta Central');
  addPortalMonument(38,0,-Math.PI/2,'guardian','Ruínas Antigas');
  addPortalMonument(0,-38,Math.PI,'cross','Cemitério');

  // Portais de entrada nas regiões.
  addPortalMonument(0,66,0,'wolf','Entrada Sul');
  addPortalMonument(-64,0,Math.PI/2,'goblin','Entrada Floresta');
  addPortalMonument(64,0,-Math.PI/2,'guardian','Entrada Ruínas');
  addPortalMonument(0,-66,Math.PI,'cross','Entrada Cemitério');

  // Cada região cercada, com apenas uma abertura alinhada à estrada.
  LORENCIA_REGIONS.forEach(r=>addRegionEnclosure(r.x,r.z,r.halfX,r.halfZ,r.gateSide));
}
function addRegionEnclosure(cx,cz,halfX,halfZ,openSide){
  const col=0x3d3834, height=6.2, thickness=2.15, gap=7.2;
  function horizontal(z, side){
    if(openSide===side){
      addWallSegment(cx-halfX,z,cx-gap,z,{height,thickness,color:col});
      addWallSegment(cx+gap,z,cx+halfX,z,{height,thickness,color:col});
    }else{
      addWallSegment(cx-halfX,z,cx+halfX,z,{height,thickness,color:col});
    }
  }
  function vertical(x, side){
    if(openSide===side){
      addWallSegment(x,cz-halfZ,x,cz-gap,{height,thickness,color:col});
      addWallSegment(x,cz+gap,x,cz+halfZ,{height,thickness,color:col});
    }else{
      addWallSegment(x,cz-halfZ,x,cz+halfZ,{height,thickness,color:col});
    }
  }
  horizontal(cz-halfZ,'north');
  horizontal(cz+halfZ,'south');
  vertical(cx-halfX,'west');
  vertical(cx+halfX,'east');
}
function isLorenciaNoNpcZone(x,z){
  if(state.world!=='lorencia') return false;
  // Praça Central e entorno são seguros.
  if(Math.hypot(x,z) < LORENCIA_SAFE_ZONE.radius + 22) return true;
  // Estradas e portais sem NPC.
  const roads=[ [0,34,0,78], [-34,0,-78,0], [34,0,78,0], [0,-34,0,-78] ];
  for(const [x1,z1,x2,z2] of roads){ if(distancePointToSegment(x,z,x1,z1,x2,z2)<14) return true; }
  const gates=[ [0,38],[-38,0],[38,0],[0,-38],[0,66],[-64,0],[64,0],[0,-66] ];
  for(const [gx,gz] of gates){ if(Math.hypot(x-gx,z-gz)<22) return true; }
  return false;
}
...
function addForestAndRocks(){
  ...
  // Lorencia já tem decoração própria por região. Evita árvores/pedras aleatórias
  // atravessando muros, portais e área de respawn.
  if(state.world==='lorencia'){
    for(let i=0;i<12;i++){
      const x=(rng()-.5)*390, z=(rng()-.5)*390;
      if(Math.abs(x)<170 && Math.abs(z)<170) continue;
      addRock(x,z,.55+rng()*1.1);
    }
    return;
  }
  for(let i=0;i<45;i++){
    const x=(rng()-.5)*450, z=(rng()-.5)*450;
    if(Math.hypot(x-LORENCIA_SAFE_ZONE.x,z-LORENCIA_SAFE_ZONE.z)<LORENCIA_SAFE_ZONE.radius+18) continue;
    addTree(x,z,.75+rng()*1.25);
  }
  for(let i=0;i<32;i++){
    const x=(rng()-.5)*430, z=(rng()-.5)*430;
    if(Math.hypot(x-LORENCIA_SAFE_ZONE.x,z-LORENCIA_SAFE_ZONE.z)<LORENCIA_SAFE_ZONE.radius+22) continue;
    addRock(x,z,.55+rng()*1.6);
  }
  for(let i=0;i<25;i++){
    const x=(rng()-.5)*420, z=(rng()-.5)*420;
    if(Math.hypot(x-LORENCIA_SAFE_ZONE.x,z-LORENCIA_SAFE_ZONE.z)<LORENCIA_SAFE_ZONE.radius+16) continue;
    addBush(x,z,.45+rng()*1.1);
  }
}
...
function setupSpawnSlots(){
  ...
  if(state.world==='lorencia'){
    LORENCIA_REGIONS.forEach((region, ridx)=>{
      const count = region.id==='south_forest' ? 7 : region.id==='central_forest' ? 7 : region.id==='ruins' ? 6 : 7;
      for(let i=0;i<count;i++){
        let x=region.x, z=region.z;
        for(let tries=0; tries<40; tries++){
          const a=rng()*Math.PI*2, r=10+rng()*(region.radius-12);
          x=region.x+Math.sin(a)*r; z=region.z+Math.cos(a)*r;
          if(!isLorenciaNoNpcZone(x,z) && canStandAt(x,z,NPC_RADIUS,null)) break;
        }
        const species=region.species[Math.floor(rng()*region.species.length)];
        const level=region.minLevel + Math.floor(rng()*(region.maxLevel-region.minLevel+1));
        const slot={id:`${region.id}_${i}`, x, z, npc:null, respawnAt:0, region:region.id, minLevel:region.minLevel, maxLevel:region.maxLevel, forcedSpecies:species, forcedLevel:level};
        slot.npc=createNpc(slot.x, slot.z, slot, species);
        spawnSlots.push(slot);
      }
    });
    log('Lorencia Sombria redesenhada: Praça Central segura, Florestas, Ruínas e Cemitério com níveis fixos por região.');
    return;
  }
...
function getNpcSpeciesForSlot(slot){
  if(slot?.forcedSpecies) return getNpcSpeciesById(slot.forcedSpecies);
...
function getNpcDamageByLevel(level){
  const table={1:50,2:75,3:100,4:150,5:200,6:275,7:350,8:450,9:550,10:700,11:900,12:1200};
  const lv=Math.max(1, Math.floor(Number(level)||1));
  return table[lv] || Math.floor(1200 + (lv-12)*180);
}
function getNpcHpByLevel(level){
  const table={1:200,2:300,3:450,4:650,5:900,6:1200,7:1600,8:2100,9:2800,10:3600,11:4500,12:5500};
  const lv=Math.max(1, Math.floor(Number(level)||1));
  return table[lv] || Math.floor(5500 + (lv-12)*650);
}
...
function createNpc(x,z,slot=null, forcedSpecies=null){
  ...
  const regionMin = Number(slot?.minLevel)||1;
  const regionMax = Math.max(regionMin, Number(slot?.maxLevel)||regionMin);
  const npcLevel = Number(slot?.forcedLevel) || (state.world==='lorencia'
    ? regionMin + Math.floor(npcRng()*(regionMax-regionMin+1))
    : Math.max(1, state.level + worldBonus + Math.floor(npcRng()*3)));
  if(label.material?.map){ label.material.map.dispose?.(); }
  label.material.map = makeTextSprite(`${spec.name}\nLv ${npcLevel}`,{color:'#ffdf8a',width:3.0,height:1.0,size:21}).material.map;
  const maxHp = Math.floor(getNpcHpByLevel(npcLevel) * spec.hpMul);
...
function publishNpcDeath(npc, slot){
  ...
  dbSet(dbRef(rtdb, `worlds/${state.world}/npcs/${npc.userData.netId}`), {
    ...
    respawnAt: Date.now()+15000, deathSeq: Date.now(), killerUid: state.uid || null, slotX:slot?.x ?? npc.position.x, slotZ:slot?.z ?? npc.position.z, updatedAt:rtdbServerTimestamp()
  }).catch(()=>{});
}
...
function startMultiplayerSync(){
  ...
  multiplayerUnsub=onValue(multiplayerPlayersRef, snap=>{
    ...
    Object.entries(data).forEach(([uid, info])=>{
      ...
      rp.userData.dead=!!info.dead;
      rp.userData.respawnSeq=Number(info.respawnSeq)||0;
      ...
      const respawnSeq=Number(info.respawnSeq)||0;
      if(!info.dead && respawnSeq && respawnSeq!==rp.userData.lastRespawnSeq){
        rp.userData.lastRespawnSeq=respawnSeq;
        rp.position.set(Number(info.x)||0, Number(info.y)||0, Number(info.z)||0);
        rp.userData.target.set(Number(info.x)||0, Number(info.y)||0, Number(info.z)||0);
        rp.rotation.z=0; rp.rotation.x=0;
...
function playRemoteAction(rp, type='attack'){
  ...
  if(type==='KeyF'){
    ...
    createRemoteSwordTrail(rp, 0x74d6ff);
  }
...
function movePlayer(delta=0.016){
  ...
  if(mobileActive){
    ...
    if(mag > .14 && Number.isFinite(mobileInput.desiredYaw)){
      player.rotation.y = lerpAngle(player.rotation.y, mobileInput.desiredYaw, 0.12);
      moveForward = mag;
    }
  } else {
  ...
  if(moving){
    ...
    const mobileSpeedFix = mobileActive ? 1.0 : 1.0;
    const frameScale = THREE.MathUtils.clamp(delta * 60, 0.5, 1.25);
    const speed = PLAYER_MOVE_SPEED * analog * mobileSpeedFix * (sprintActive && moveForward > 0 ? SPRINT_MULTIPLIER : 1) * frameScale;
...
function attack(){
  if(playerDead || !player || isAttacking) return;
...
function specialAttack(code){
  if(playerDead || !player || skillLock) return;
...
function startPlayerDeath(){
  ...
  setTimeout(()=>{
    ...
    playerDead=false;
    playerRespawnSeq=Date.now();
    setTimeout(()=>{ if(player) player.visible=true; }, 80);
    syncOwnPlayerOnline();
    updateHud();
  },2300);
}
...
function createRemoteSwordTrail(rp, color=0x70d7ff){
  if(!rp || !scene) return;
  const forward=new THREE.Vector3(Math.sin(rp.rotation.y),0,Math.cos(rp.rotation.y));
  const right=new THREE.Vector3(Math.cos(rp.rotation.y),0,-Math.sin(rp.rotation.y));
  const pos=rp.position.clone().addScaledVector(forward,1.25).addScaledVector(right,.55).add(new THREE.Vector3(0,1.55,0));
  const trail=new THREE.Mesh(new THREE.TorusGeometry(.78,.025,8,36,Math.PI*1.25),new THREE.MeshBasicMaterial({color, transparent:true, opacity:.45, side:THREE.DoubleSide}));
  trail.position.copy(pos); trail.rotation.set(Math.PI/2,0,rp.rotation.y+.6); scene.add(trail);
  visualEffects.push({mesh:trail, life:.22, age:0, type:'trail'});
}
...
function startDropRealtimeSync(){
  if(!USE_FIREBASE || !state.uid || !state.world) return;
  stopDropRealtimeSync();
  multiplayerDropsRef=dbRef(rtdb, `worlds/${state.world}/drops`);
  multiplayerDropsUnsub=onValue(multiplayerDropsRef, snap=>{
    const data=snap.val()||{};
    const seen=new Set(Object.keys(data));
    Object.entries(data).forEach(([id, info])=>{
      if(!info || info.collectedBy) return;
      const createdAt=Number(info.createdAt)||Date.now();
      if(Date.now()-createdAt > 60000){
        if(isNpcHost) dbRemove(dbRef(rtdb, `worlds/${state.world}/drops/${id}`)).catch(()=>{});
        return;
      }
      let existing=coinPickups.find(c=>c.userData?.netId===id);
      if(!existing){
        createCoinDrop(new THREE.Vector3(Number(info.x)||0,0,Number(info.z)||0), Number(info.amount)||0, {netId:id, shared:true, born:performance.now()-(Date.now()-createdAt)});
      }
    });
    for(let i=coinPickups.length-1;i>=0;i--){
      const c=coinPickups[i];
      if(c.userData?.shared && c.userData?.netId && !seen.has(c.userData.netId)){
        if(c.parent) scene.remove(c);
        coinPickups.splice(i,1);
      }
    }
  }, err=>console.warn('Drops em tempo real sem permissão:', err));
  dropsCleanupTimer=setInterval(()=>{
    if(!isNpcHost || !state.world) return;
    // Limpeza leve feita pelo host: os drops também somem localmente se passarem de 60s.
  }, 10000);
}
function stopDropRealtimeSync(){
  if(dropsCleanupTimer){ clearInterval(dropsCleanupTimer); dropsCleanupTimer=null; }
  if(multiplayerDropsUnsub){ try{ multiplayerDropsUnsub(); }catch(e){} multiplayerDropsUnsub=null; }
  multiplayerDropsRef=null;
}
function publishSharedDrop(pos, amount, npcId='drop'){
  if(!USE_FIREBASE || !state.world || !multiplayerDropsRef) return;
  const id=`${npcId}_${Date.now()}_${Math.floor(Math.random()*9999)}`;
  dbSet(dbRef(rtdb, `worlds/${state.world}/drops/${id}`), {
    id, type:'coins', amount:Number(amount)||0,
    x:Number(pos.x.toFixed(3)), z:Number(pos.z.toFixed(3)),
    createdAt:Date.now(), killerUid:state.uid||null
  }).catch(()=>{});
}
async function collectSharedDrop(drop){
  if(!drop?.userData?.netId || !state.world || drop.userData.collecting) return;
  drop.userData.collecting=true;
  const dropRef=dbRef(rtdb, `worlds/${state.world}/drops/${drop.userData.netId}`);
  try{
    const result=await runTransaction(dropRef, current=>{
      if(!current || current.collectedBy) return current;
      return {...current, collectedBy:state.uid||'local', collectedAt:Date.now()};
    });
    if(result.committed && result.snapshot.val()?.collectedBy===(state.uid||'local')){
      state.coins += drop.userData.amount;
      log(`Você coletou ${drop.userData.amount} moedas.`);
      createFloatingText('+'+drop.userData.amount, player.position.clone().add(new THREE.Vector3(0,1.8,0)), '#ffdd55');
      createHitEffect(player.position.clone(), .75, 0xffd84d);
      updateHud(); scheduleSaveProgress();
      dbRemove(dropRef).catch(()=>{});
    }
  }catch(e){ console.warn('Falha ao coletar drop:', e); }
  finally{ drop.userData.collecting=false; }
}

function createCoinDrop(pos, amount, opts={}){
  if(!scene) return;
  const group=new THREE.Group();
  ...
  group.position.y=.18;
  group.userData={amount, born:opts.born ?? performance.now(), netId:opts.netId||null, shared:!!opts.shared, collecting:false};
  scene.add(group);
...
function processCoinPickups(){
  ...
  for(let i=coinPickups.length-1;i>=0;i--){
    ...
    if(age>60000){
      if(c.userData.shared && c.userData.netId && isNpcHost && state.world){ dbRemove(dbRef(rtdb, `worlds/${state.world}/drops/${c.userData.netId}`)).catch(()=>{}); }
      scene.remove(c); coinPickups.splice(i,1); continue;
    }
    const d=c.position.distanceTo(player.position);
    if(d<1.15 && !playerDead){
      if(c.userData.shared && c.userData.netId){
        collectSharedDrop(c);
      } else {
        ...
        createHitEffect(player.position.clone(), .75, 0xffd84d);
        scene.remove(c); coinPickups.splice(i,1); updateHud(); scheduleSaveProgress();
      }
    }
  }
}
...
function killNpc(npc){
  ...
  publishSharedDrop(npc.position.clone(), currentWorld.coins, npc.userData.netId || 'npc');
  log(`NPC derrotado: +${currentWorld.xp} XP. Drop de moedas caiu no chão.`);
  let leveled=false;
...
function stopGame(){ stopDropRealtimeSync(); stopNpcRealtimeSync(); stopMultiplayerSync(true); savePlayerProgressNow(); if(animId) cancelAnimationFrame(animId); actionTimers.forEach(id=>clearInterval(id)); actionTimers=[]; if(renderer){ renderer.dispose(); } npcs=[]; coinPickups=[]; visualEffects=[]; colliders=[]; spawnSlots=[]; scene=null; camera=null; renderer=null; player=null; isAttacking=false; skillLock=false; hudHidden=false; walkClock=0; lastFrameTime=0; cameraYaw=0; cameraLockedDuringSkill=false; rightMouseDown=false; lockedTarget=null; targetIndicator=null; sprintActive=false; sprintEndAt=0; sprintCooldownUntil=0; multiplayerActionSeq=0; lastActionType='idle'; lastActionAt=0; playerRespawnSeq=0; playerDead=false; deathStartedAt=0; cameraShakeUntil=0; cameraShakePower=0; keys={}; document.body.classList.remove('hud-hidden'); $('combatLog').innerHTML=''; }
