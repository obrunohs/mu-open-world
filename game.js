...
const LORENCIA_SAFE_ZONE = { id:'safe', name:'Praça Central', subtitle:'Área Segura', x:0, z:0, radius:34 };
const LORENCIA_REGIONS = [
  { id:'south_forest', name:'Floresta Sul', subtitle:'NPC nível 1-3', minLevel:1, maxLevel:3, species:['goblin','spider'], x:0, z:82, radius:54, color:0x224d25 },
  { id:'central_forest', name:'Floresta Central', subtitle:'NPC nível 3-5', minLevel:3, maxLevel:5, species:['goblin','wolf','spider'], x:-88, z:6, radius:58, color:0x173d21 },
  { id:'ruins', name:'Ruínas Antigas', subtitle:'NPC nível 5-8', minLevel:5, maxLevel:8, species:['goblin','wolf'], x:86, z:-12, radius:58, color:0x574f46 },
  { id:'cemetery', name:'Cemitério Maldito', subtitle:'NPC nível 8-12', minLevel:8, maxLevel:12, species:['wolf','spider','goblin'], x:0, z:-96, radius:62, color:0x2b2832 }
];
...
function addLorenciaRegions(){
  ...
  const rng=createSeededRandom('lorencia-region-decor');
  // Praça Central: área neutra com tochas, corvos e pontos futuros de trade.
  for(let i=0;i<8;i++){ const a=i*Math.PI*2/8; addTorch(Math.sin(a)*24, Math.cos(a)*24); }
  [[-15,12,1.8],[15,14,-1.7],[-20,-8,.7],[20,-10,-.8],[5,29,3.1]].forEach(([x,z,r])=>addCrow(x,z,r));
  const safeRing=new THREE.Mesh(new THREE.RingGeometry(LORENCIA_SAFE_ZONE.radius-.35, LORENCIA_SAFE_ZONE.radius+.35,96), new THREE.MeshBasicMaterial({color:0x82d7ff, transparent:true, opacity:.18, side:THREE.DoubleSide}));
  safeRing.rotation.x=-Math.PI/2; safeRing.position.y=.035; scene.add(safeRing);
  // Floresta Sul: árvores abertas, cogumelos, troncos caídos.
  for(let i=0;i<14;i++){ const x=(rng()-.5)*80, z=62+rng()*65; if(Math.hypot(x,z)<38) continue; addTree(x,z,.6+rng()*.75); }
  for(let i=0;i<35;i++) addMushroom((rng()-.5)*85,55+rng()*70,.7+rng()*1.4);
  for(let i=0;i<7;i++){ const log=new THREE.Mesh(new THREE.CylinderGeometry(.28,.34,3.8,8),new THREE.MeshStandardMaterial({color:0x4d2f19,roughness:.9})); log.rotation.z=Math.PI/2; log.rotation.y=rng()*Math.PI; log.position.set((rng()-.5)*78,.35,55+rng()*75); scene.add(log); addCollider(log.position.x,log.position.z,1.8,'fallen_log',log); }
  // Floresta Central: árvores densas, barris e acampamento abandonado.
  for(let i=0;i<18;i++){ const a=rng()*Math.PI*2, r=15+rng()*47; addTree(-88+Math.sin(a)*r,6+Math.cos(a)*r,.85+rng()*1.05); }
  for(let i=0;i<9;i++){ const barrel=new THREE.Mesh(new THREE.CylinderGeometry(.42,.42,.8,12),new THREE.MeshStandardMaterial({color:0x5a351b,roughness:.78})); barrel.position.set(-110+rng()*45,.4,-20+rng()*55); barrel.rotation.z=(rng()-.5)*.35; scene.add(barrel); addCollider(barrel.position.x,barrel.position.z,.55,'barrel',barrel); }
  addTorch(-86,-18); addTorch(-112,20); addCrow(-96,28,-.5);
  // Ruínas: colunas, estátuas quebradas e fogo azul.
  for(let i=0;i<16;i++){ const a=rng()*Math.PI*2, r=10+rng()*45; addBrokenColumn(86+Math.sin(a)*r,-12+Math.cos(a)*r,.65+rng()*.8); }
  for(let i=0;i<5;i++){ const torchX=65+rng()*45, torchZ=-38+rng()*52; addTorch(torchX,torchZ); const last=decorAnimatables[decorAnimatables.length-1]; if(last?.flame){ last.flame.material.color.setHex(0x3ca7ff); last.flame2.material.color.setHex(0xb9f2ff); last.light.color.setHex(0x4fb9ff); } }
  // Cemitério: túmulos, árvores mortas, corvos e névoa escura local.
  for(let i=0;i<32;i++){ const a=rng()*Math.PI*2, r=8+rng()*55; addTomb(Math.sin(a)*r,-96+Math.cos(a)*r,rng()*Math.PI); }
  for(let i=0;i<9;i++){ const a=rng()*Math.PI*2, r=18+rng()*55; const x=Math.sin(a)*r, z=-96+Math.cos(a)*r; const dead=new THREE.Mesh(new THREE.CylinderGeometry(.18,.26,3.2,7),new THREE.MeshStandardMaterial({color:0x2b201b,roughness:.95})); dead.position.set(x,1.6,z); dead.rotation.z=(rng()-.5)*.28; scene.add(dead); addCollider(x,z,.45,'dead_tree',dead); }
  [[-18,-82,.2],[22,-110,-.9],[8,-139,1.5],[-34,-124,2.3]].forEach(([x,z,r])=>addCrow(x,z,r));
}
...
function addPortalMonument(x,z,dir,type,labelText){
  const group=new THREE.Group();
  const stone=new THREE.MeshStandardMaterial({color:0x716961, roughness:.86});
  const dark=new THREE.MeshStandardMaterial({color:0x2e2827, roughness:.92});
  const glow=new THREE.MeshBasicMaterial({color:0x86caff, transparent:true, opacity:.28, side:THREE.DoubleSide});
  const base=new THREE.Mesh(new THREE.BoxGeometry(9,.7,2.5),stone); base.position.y=.35; group.add(base);
  [-3.2,3.2].forEach(px=>{ const p=new THREE.Mesh(new THREE.BoxGeometry(1.4,5.8,1.4),stone); p.position.set(px,3.25,0); group.add(p); });
  const top=new THREE.Mesh(new THREE.BoxGeometry(8.2,1.1,1.8),stone); top.position.y=6.2; group.add(top);
  const ring=new THREE.Mesh(new THREE.TorusGeometry(2.15,.12,8,32),glow); ring.position.y=3.35; ring.rotation.y=Math.PI/2; group.add(ring);
  if(type==='wolf'){
  ...
  // Sem collider no centro do portal para manter a entrada aberta.
...
function addLorenciaWallsAndPortals(){
  // Muro externo alto da cidade/mapa.
  const m=228;
  ...
  // Praça Central murada com 4 entradas abertas.
  const h=39, gap=9;
  addWallSegment(-h,-h,-gap,-h,{height:7.8,thickness:2.4}); addWallSegment(gap,-h,h,-h,{height:7.8,thickness:2.4});
  addWallSegment(-h,h,-gap,h,{height:7.8,thickness:2.4}); addWallSegment(gap,h,h,h,{height:7.8,thickness:2.4});
  addWallSegment(-h,-h,-h,-gap,{height:7.8,thickness:2.4}); addWallSegment(-h,gap,-h,h,{height:7.8,thickness:2.4});
  addWallSegment(h,-h,h,-gap,{height:7.8,thickness:2.4}); addWallSegment(h,gap,h,h,{height:7.8,thickness:2.4});

  // Estradas sem spawn, visuais, saindo da safe zone.
  addRoadSegment(0,40,0,82,11);       // Floresta Sul
  addRoadSegment(-40,0,-88,6,11);     // Floresta Central
  addRoadSegment(40,0,86,-12,11);     // Ruínas
  addRoadSegment(0,-40,0,-96,11);     // Cemitério

  // Portais/entradas monumentais ao redor do muro central.
  addPortalMonument(0,43,0,'wolf','Floresta Sul');
  addPortalMonument(-43,0,Math.PI/2,'goblin','Floresta Central');
  addPortalMonument(43,0,-Math.PI/2,'guardian','Ruínas Antigas');
  addPortalMonument(0,-43,Math.PI,'cross','Cemitério');

  // Cercas/muros de cada região com abertura voltada para a praça.
  addRegionEnclosure(0,82,72,58,'north');
  addRegionEnclosure(-88,6,60,58,'east');
  addRegionEnclosure(86,-12,60,58,'west');
  addRegionEnclosure(0,-96,72,62,'south');
}
function addRegionEnclosure(cx,cz,halfX,halfZ,openSide){
  const gap=15, col=0x3d3834;
  if(openSide!=='north'){ addWallSegment(cx-halfX,cz-halfZ,cx-gap,cz-halfZ,{height:5.8,thickness:2,color:col}); addWallSegment(cx+gap,cz-halfZ,cx+halfX,cz-halfZ,{height:5.8,thickness:2,color:col}); }
  if(openSide!=='south'){ addWallSegment(cx-halfX,cz+halfZ,cx-gap,cz+halfZ,{height:5.8,thickness:2,color:col}); addWallSegment(cx+gap,cz+halfZ,cx+halfX,cz+halfZ,{height:5.8,thickness:2,color:col}); }
  if(openSide!=='west'){ addWallSegment(cx-halfX,cz-halfZ,cx-halfX,cz-gap,{height:5.8,thickness:2,color:col}); addWallSegment(cx-halfX,cz+gap,cx-halfX,cz+halfZ,{height:5.8,thickness:2,color:col}); }
  if(openSide!=='east'){ addWallSegment(cx+halfX,cz-halfZ,cx+halfX,cz-gap,{height:5.8,thickness:2,color:col}); addWallSegment(cx+halfX,cz+gap,cx+halfX,cz+halfZ,{height:5.8,thickness:2,color:col}); }
}
function isLorenciaNoNpcZone(x,z){
  if(state.world!=='lorencia') return false;
  if(Math.hypot(x,z) < LORENCIA_SAFE_ZONE.radius + 20) return true;
  const roads=[ [0,40,0,96], [-40,0,-102,8], [40,0,100,-15], [0,-40,0,-112] ];
  for(const [x1,z1,x2,z2] of roads){ if(distancePointToSegment(x,z,x1,z1,x2,z2)<13) return true; }
  const gates=[ [0,43],[-43,0],[43,0],[0,-43] ];
  for(const [gx,gz] of gates){ if(Math.hypot(x-gx,z-gz)<24) return true; }
  return false;
}
...
function addForestAndRocks(){
  ...
  for(let i=0;i<45;i++){
    ...
    if(Math.hypot(x,z)<LORENCIA_SAFE_ZONE.radius+18) continue;
