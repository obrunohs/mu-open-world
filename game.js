...
function addTomb(x,z,rot=0){
  ...
  const base=new THREE.Mesh(new THREE.BoxGeometry(.95,.2,1.55),mat); base.position.set(x,.12,z); base.rotation.y=rot; scene.add(base); addCollider(x,z,.8,'tomb',base);
...
function addLorenciaRegions(){
  ...
  for(let i=0;i<28;i++){ const x=(rng()-.5)*80, z=62+rng()*65; if(Math.hypot(x,z)<38) continue; addTree(x,z,.6+rng()*.75); }
  ...
  for(let i=0;i<38;i++){ const a=rng()*Math.PI*2, r=15+rng()*47; addTree(-88+Math.sin(a)*r,6+Math.cos(a)*r,.85+rng()*1.05); }
