...
console.log('MU PDL Survival build: v18 - Mobile HUD/Controls + Realtime Actions');
...
let multiplayerActionSeq=0, lastActionType='idle', lastActionAt=0;
let mobileInput={active:false,x:0,y:0,forward:0,turn:0};
...
function setupMobileControls(){
  ...
  function resetStick(){ keys.KeyW=false; keys.KeyS=false; mobileInput={active:false,x:0,y:0,forward:0,turn:0}; if(knob) knob.style.transform='translate(0px,0px)'; stickId=null; }
  if(stickZone){
    ...
    stickZone.addEventListener('touchmove',e=>{
      ...
      mobileInput.y=ay;
      mobileInput.turn=ax;
      mobileInput.forward=-ay;
      keys.KeyW = mobileInput.forward > .22;
      keys.KeyS = mobileInput.forward < -.22;
      e.preventDefault();
    }, {passive:false});
...
function setupSpawnSlots(){
  ...
  spawnPoints.forEach(([x,y,z],idx)=>{
    ...
    for(let i=0;i<count;i++){
      const slot={id:`npc_${idx}_${i}`, x:x+(Math.random()*8-4), z:z+(Math.random()*8-4), npc:null, respawnAt:0};
...
function createNpc(x,z,slot=null){
  ...
  Object.assign(npc.userData,{netId:slot?.id||null, level:npcLevel, damage:getNpcDamageByLevel(npcLevel), attackDelay:spec.attackDelay, lastHit:0, radius:spec.radius, hp:maxHp,maxHp,hpBar,hpGroup,label,slot,walkTime:Math.random()*10,attackAnim:0,attackUntil:0,baseY:0,attackDir:null});
...
function startNpcRealtimeSync(){
  ...
  multiplayerNpcsUnsub=onValue(multiplayerNpcsRef, snap=>{
    ...
    if(Object.keys(data).length===0){
      // Primeiro jogador online popula os NPCs iniciais para todos verem iguais.
...
function publishNpcDeath(npc, slot){
  ...
  dbSet(dbRef(rtdb, `worlds/${state.world}/npcs/${npc.userData.netId}`), {
    id: npc.userData.netId, alive:false, species:npc.userData.species, hp:0, maxHp:npc.userData.maxHp, level:npc.userData.level,
    x:Number(npc.position.x.toFixed(3)), z:Number(npc.position.z.toFixed(3)), rot:Number(npc.rotation.y.toFixed(4)),
    respawnAt: Date.now()+15000, slotX:slot?.x ?? npc.position.x, slotZ:slot?.z ?? npc.position.z, updatedAt:rtdbServerTimestamp()
  }).catch(()=>{});
}
function applySharedNpcState(id, info){
  ...
  if(info.alive===false){
    ...
    if(slot.lastDeathKey!==deathKey){
      ...
      if(scene && !isNpcHost){ createCoinDrop(new THREE.Vector3(Number(info.x)||slot.x,0,Number(info.z)||slot.z), currentWorld.coins); }
    }
  ...
  if(!slot.npc){ slot.npc=createNpc(Number(info.x)||slot.x, Number(info.z)||slot.z, slot); }
  const n=slot.npc;
  n.position.set(Number(info.x)||n.position.x, Number(info.y)||0, Number(info.z)||n.position.z);
  n.rotation.y=Number(info.rot)||n.rotation.y;
...
function movePlayer(delta=0.016){
  ...
  if(mobileActive){
    // No celular o joystick vira o personagem e a câmera acompanha suavemente.
    const turn = THREE.MathUtils.clamp(mobileInput.turn || 0, -1, 1);
    player.rotation.y -= turn * PLAYER_TURN_SPEED * 1.05;
  } else {
  ...
  let moveForward = 0;
  if(mobileActive){
    moveForward = Math.abs(mobileInput.forward) > .22 ? THREE.MathUtils.clamp(mobileInput.forward, -1, 1) : 0;
  } else {
  ...
  if(moving){
    const analog = mobileActive ? Math.max(.55, Math.min(1, Math.abs(moveForward))) : 1;
...
function moveNpcs(){
  if(multiplayerNpcsRef && !isNpcHost){ npcs.forEach(n=>{ animateNpcWalk(n,false); animateNpcAttack(n); }); return; }
  ...
  npcs.forEach(n=>{
    ...
    if(multiplayerNpcsRef && isNpcHost && Math.random()<0.18) publishNpcState(n);
  });
}
