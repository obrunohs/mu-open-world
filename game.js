...
console.log('MU PDL Survival build: v19 - Mobile joystick + Full Realtime Fix');
...
function setupMobileControls(){
  ...
  function resetStick(){ keys.KeyW=false; keys.KeyS=false; mobileInput={active:false,x:0,y:0,forward:0,turn:0,mag:0}; if(knob) knob.style.transform='translate(0px,0px)'; stickId=null; }
  if(stickZone){
    ...
    stickZone.addEventListener('touchmove',e=>{
      ...
      mobileInput.mag=Math.min(1, Math.hypot(dx,dy)/max);
...
function publishNpcState(npc){
  ...
  dbSet(dbRef(rtdb, `worlds/${state.world}/npcs/${ud.netId}`), {
    ...
    attackSeq: ud.attackSeq||0, attacking: !!ud.attackAnim,
...
function applySharedNpcState(id, info){
  ...
  n.userData.netMoving = Math.hypot(nx-n.position.x, nz-n.position.z) > NPC_REMOTE_MOVE_EPSILON;
  n.position.set(nx, ny, nz);
  n.rotation.y=Number(info.rot)||n.rotation.y;
  const attackSeq=Number(info.attackSeq)||0;
  if(attackSeq && attackSeq!==n.userData.lastRemoteAttackSeq){ n.userData.lastRemoteAttackSeq=attackSeq; startNpcAttack(n, false); }
...
function syncOwnPlayerOnline(){
  ...
  dbSet(multiplayerMyRef, {
    ...
    maxHp: state.maxHp,
    moving: !!(keys.KeyW || keys.KeyS || (mobileInput.active && (mobileInput.mag||0)>.16)),
...
function movePlayer(delta=0.016){
  ...
  let moveForward = 0;
  if(mobileActive){
    // Mobile: o joystick define o ângulo do movimento; o personagem e a câmera acompanham suavemente.
    const mag = THREE.MathUtils.clamp(mobileInput.mag || Math.hypot(mobileInput.x||0, mobileInput.forward||0), 0, 1);
    if(mag > .16){
      const desiredYaw = cameraYaw + Math.atan2(mobileInput.x || 0, mobileInput.forward || 0);
...
function moveNpcs(){
  if(multiplayerNpcsRef && !isNpcHost){ npcs.forEach(n=>{ animateNpcWalk(n,!!n.userData.netMoving); animateNpcAttack(n); n.userData.netMoving=false; }); return; }
