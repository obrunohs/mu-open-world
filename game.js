...
console.log('MU PDL Survival build: v20 - Global sync + joystick stable');
...
function setupMobileControls(){
  ...
  function resetStick(){ keys.KeyW=false; keys.KeyS=false; keys.KeyA=false; keys.KeyD=false; mobileInput={active:false,x:0,y:0,forward:0,turn:0,mag:0}; if(knob) knob.style.transform='translate(0px,0px)'; stickId=null; }
  if(stickZone){
    ...
    stickZone.addEventListener('touchmove',e=>{
      ...
      keys.KeyA = mobileInput.turn < -.28;
      keys.KeyD = mobileInput.turn > .28;
      e.preventDefault();
    }, {passive:false});
...
function syncOwnPlayerOnline(){
  ...
  dbSet(multiplayerMyRef, {
    ...
    maxHp: state.maxHp,
    moving: !!(keys.KeyW || keys.KeyS || (mobileInput.active && Math.abs(mobileInput.forward||0)>.18)),
...
function movePlayer(delta=0.016){
  ...
  let moveForward = 0;
  if(mobileActive){
    // Mobile estável: eixo X gira o personagem/câmera suavemente; eixo Y anda para frente/trás.
    const turn = THREE.MathUtils.clamp(mobileInput.turn || 0, -1, 1);
    const forwardAxis = THREE.MathUtils.clamp(mobileInput.forward || 0, -1, 1);
    if(Math.abs(turn) > .12) player.rotation.y -= turn * PLAYER_TURN_SPEED * 1.12;
    if(Math.abs(forwardAxis) > .18) moveForward = forwardAxis;
  } else {
