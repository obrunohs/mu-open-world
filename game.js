...
function setupMobileControls(){
  ...
  const leftBtn=$('mobileTurnLeft');
  const rightBtn=$('mobileTurnRight');
  ...
  bindHold(leftBtn,'KeyA');
  bindHold(rightBtn,'KeyD');
  let stickId=null, center={x:0,y:0};
  function resetStick(){ keys.KeyW=false; keys.KeyS=false; if(knob) knob.style.transform='translate(0px,0px)'; stickId=null; }
  if(stickZone){
    stickZone.addEventListener('touchstart',e=>{
      if(stickId!==null) return;
      const t=e.changedTouches[0]; stickId=t.identifier;
      const rect=stickZone.getBoundingClientRect();
      center={x:rect.left+96, y:rect.bottom-84};
      e.preventDefault();
    }, {passive:false});
...
function startGame(){
  ...
  addWorldEnvironment(); createPlayer(); setupSpawnSlots(); startMultiplayerSync(); animate(); resizeGameRenderer();
}
...
function stopGame(){ stopMultiplayerSync(true); savePlayerProgressNow(); if(animId) cancelAnimationFrame(animId); actionTimers.forEach(id=>clearInterval(id)); actionTimers=[]; if(renderer){ renderer.dispose(); } npcs=[]; coinPickups=[]; visualEffects=[]; colliders=[]; spawnSlots=[]; scene=null; camera=null; renderer=null; player=null; isAttacking=false; skillLock=false; hudHidden=false; walkClock=0; lastFrameTime=0; cameraYaw=0; cameraLockedDuringSkill=false; rightMouseDown=false; lockedTarget=null; targetIndicator=null; sprintActive=false; sprintEndAt=0; sprintCooldownUntil=0; multiplayerActionSeq=0; lastActionType='idle'; lastActionAt=0; playerDead=false; deathStartedAt=0; cameraShakeUntil=0; cameraShakePower=0; keys={}; document.body.classList.remove('hud-hidden'); $('combatLog').innerHTML=''; }
